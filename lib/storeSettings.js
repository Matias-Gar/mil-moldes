...
export function resolveBranchWhatsapp(activeSucursal, storeSettings = null) {
  const branchPhone = cleanWhatsappNumber(activeSucursal?.telefono);
  if (branchPhone) return branchPhone;
  return cleanWhatsappNumber(storeSettings?.whatsapp_number || DEFAULT_STORE_SETTINGS.whatsapp_number);
}
