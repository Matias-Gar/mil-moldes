...
export const DESCUENTO_POR_MAYOR = {
  tipo: 'descuento_por_mayor',
  valor: 5,
  descripcion: 'Descuento al por mayor',
  cantidad_minima: 3,
};

const getCantidadBaseProducto = (producto) => {
  const parsed = Number(producto?.cantidad_base ?? producto?.cantidad ?? 0);
  return Number.isFinite(parsed) ? Math.max(0, parsed) : 0;
};

const getPrecioBaseProducto = (producto) => {
  const candidates = [
...
export const calcularPrecioConPromocion = (producto, promociones) => {
  ...
  const precioBase = getPrecioBaseProducto(producto);

  const cantidadBase = getCantidadBaseProducto(producto);
  const aplicaPorMayor = cantidadBase >= DESCUENTO_POR_MAYOR.cantidad_minima;

  if (!promocionActiva && !aplicaPorMayor) {
    return {
  ...
  if (aplicaPorMayor) {
    precioFinal = precioFinal * (1 - DESCUENTO_POR_MAYOR.valor / 100);
  }
  ...
  const promocion = aplicaPorMayor
    ? {
        ...(promocionActiva || {}),
        tipo: promocionActiva ? `${promocionActiva.tipo}+${DESCUENTO_POR_MAYOR.tipo}` : DESCUENTO_POR_MAYOR.tipo,
        descripcion: promocionActiva?.descripcion
          ? `${promocionActiva.descripcion} + ${DESCUENTO_POR_MAYOR.descripcion}`
          : DESCUENTO_POR_MAYOR.descripcion,
        descuento_por_mayor: DESCUENTO_POR_MAYOR,
      }
    : promocionActiva;
