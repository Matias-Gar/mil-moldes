...
export default function PagosTransferenciasPage() {
  const { loading, error, stats, rows, chartData, prevChartData, growth, insight, kpis, diasSinVentas } = useMetodoStats("transfer", "Transferencia", ["transferencia", "transfer"]);
    ...
    <div style={{ maxWidth: 950, margin: "0 auto", padding: 24 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, marginBottom: 24 }}>
        <div style={{ background: "#fff", borderRadius: 12, padding: 20, boxShadow: "0 1px 4px #eee" }}>
          <div style={{ fontSize: 15, color: "#888" }}>Total Transferencia</div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Pagos recientes (Transferencia)</CardTitle>
        ...
        <CardContent>
          {loading ? (
            ...
            <div>No hay pagos por transferencia en este periodo</div>
          ) : (
